...
//caculate q=fh(v);
NTL::ZZ Key::genHmacQ(SecByteBlock secretKey_h, string authority){
	string mac, encoded;
	SecByteBlock newKey = secretKey_h;
	HMAC< SHA256 > hmac(newKey, newKey.size());

	StringSource ss2(authority, true, new HashFilter(hmac, new StringSink(mac))); // StringSource
	encoded.clear();
	StringSource ss3(mac, true, new HexEncoder(new StringSink(encoded))); // StringSource
	NTL::ZZ bigNumber;
	bigNumber = NTL::to_ZZ(0);
	bigNumber = covHextoBigNumber(encoded);
	return bigNumber;
}
//------------------------------------------------------------------------
