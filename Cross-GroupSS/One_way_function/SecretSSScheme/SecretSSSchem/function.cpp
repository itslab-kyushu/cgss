...
void outSecKey(SecByteBlock h_sec, Var& varia){
	unsigned char outArray[KeySize / 8];
	for (int i = 0; i < h_sec.size(); i++){
		outArray[i] = h_sec[i];
	}
	ofstream outfile(varia.KeyPath, ios::binary);
	if (!outfile){
	...
	outfile.write((char*)& outArray, (KeySize / 8));
	outfile.clear();
	outfile.close();
}
SecByteBlock readSecKey(Var& varia){
	...
	unsigned char outArray[KeySize / 8];
	infile.read((char*)& outArray, (KeySize / 8));		//read size 以字节为单位读入；
	infile.clear();
	infile.close();
	SecByteBlock h_sec(KeySize / 8);
	for (int i = 0; i < h_sec.size(); i++){
		h_sec[i] = outArray[i];
	}
	return h_sec;
