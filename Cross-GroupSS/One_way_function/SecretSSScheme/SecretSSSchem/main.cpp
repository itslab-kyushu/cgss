...
int main(){
	...
	//--------------------------------------------------------------------------------------------------
		start = clock();
		SecByteBlock h_sec = linshiKey.genKey_Sec();						//生成用户密钥h;
		string h_str = linshiKey.covSecKeyToString(h_sec);
		BigNumber h = linshiKey.covHextoBigNumber(h_str);
		end = clock();
		outSecKey(h_sec, varia);											//输出密钥到硬盘上
		outPutTime(start, end, "生成用户密钥h", genSecretHTime);		
	//--------------------------------------------------------------------------------------------------
	//			生成q=fh(v);  一方関数fh(x)は HMACを使います。
	//--------------------------------------------------------------------------------------------------
		start = clock();
		BigNumber q = linshiKey.genHmacQ(h_sec, v_str);						//计算一方函数q=fh(v)的型，输入的两数据一个是SecByte类型，一个是string类型的十六进制数
		end = clock();
		outPutTime(start, end, "生成hash q", genHashTime);
		v_str.~string();
	//--------------------------------------------------------------------------------------------------
	//			从硬盘写入内存
	//--------------------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	//					从vi恢复v;并使用q=fh(v)进行计算；
	//----------------------------------------------------------------------------------------
		ShamirSSScheme ReAuthorityTool;
		vector<BigNumber> FileAuthShare_Set;
		readAuthrotiy(FileAuthShare_Set, varia);
		start = clock();
		secretReconstruction_V(ReAuthorityTool, m, t, prime, AuthorityShare_Set, y_id);	//对权限秘密v进行复元；
		end = clock();
		...
		SecByteBlock re_h_sec = readSecKey(varia);										//读入密钥
		BigNumber q_re = linshiKey.genHmacQ(re_h_sec, v_re_str);							//将复元之后得到的v进行q=fh(v)运算
